			...
			if (playeringame[data.match.num[i]])
			{
				...
				else
				{
					...
					else if (inttype == int_race)
					{
						...
						{
							if (data.match.numplayers > 9)
								_snprintf(strtime, sizeof strtime, "TIME O.");
							else
								_snprintf(strtime, sizeof strtime, "TIME OVER");
						}
						else if (players[data.match.num[i]].lives <= 0)
						{
							if (data.match.numplayers > 9)
								_snprintf(strtime, sizeof strtime, "GAME O.");
							else
								_snprintf(strtime, sizeof strtime, "GAME OVER");
						}
						else
							_snprintf(strtime, sizeof strtime, "%i:%02i.%02i", G_TicsToMinutes(data.match.scores[i], true),
									G_TicsToSeconds(data.match.scores[i]), G_TicsToCentiseconds(data.match.scores[i]));
...
void Y_StartIntermission(void)
{
	...
	switch (inttype)
	{
		...
		case int_match:
		{
			...
			if (mapheaderinfo[prevmap].actnum)
				_snprintf(data.match.levelstring,
		...
		case int_classicrace: // classic (full race)
		{
			...
			// set up the levelstring
			if (mapheaderinfo[prevmap].actnum)
				_snprintf(data.race.levelstring,
...
static void Y_FollowIntermission(void)
{
	...
	{
		char nameofdemo[256];
		_snprintf(nameofdemo, sizeof nameofdemo,
